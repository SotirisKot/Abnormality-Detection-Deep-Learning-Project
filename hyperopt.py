...
In this file we run hyperopt to finetune hyperparameters
...
def init_the_logger(hdlr):
    ...
    # FOR HYPEROPT
    hdlr = logging.FileHandler(os.path.join(odir_checkpoint, 'hyperopt_tests.log'))
...
def run_model(search_space):
    ...
    results_hyperopt = None
    if os.path.exists('/mnt/data/sotiris/results_hyperopt.json'):
        with open('/mnt/data/sotiris/results_hyperopt.json') as fi:
            results_hyperopt = json.load(fi)
    else:
    ...
    if patience != 0 and epoch != epochs - 1:
        ...
        for epoch in range(epoch + 1, epochs):
            ...
            best_checkpoint_name = 'hyperopt_trial_{}.pth.tar'.format(trials.tids[-1])
            ...
            if model_name not in results_hyperopt.keys():
                results_hyperopt[model_name] = dict()
            if trials.tids[-1] not in results_hyperopt[model_name].keys():
                results_hyperopt[model_name][trials.tids[-1]] = dict()
                results_hyperopt[model_name][trials.tids[-1]]['hyperparameters'] = trials.trials[-1]['misc']['vals']
                results_hyperopt[model_name][trials.tids[-1]]['results'] = list()
            results_hyperopt[model_name][trials.tids[-1]]['results'].append({
                'epoch': epoch,
                'patience': patience,
            ...
            with open('/mnt/data/sotiris/results_hyperopt.json', 'w') as out:
                json.dump(results_hyperopt, out)
        ...
        # We let the model train with a certain search space and then hyperopt optimizes based on best_auc
        # We want to improve best_auc
        # We use negative because hyperopts tries to minimize
        return {
            'loss': -best_auc,
            'status': STATUS_OK,
        }
...
# Initialize search space and run hyperopt
...
# Set if you want to resume training
resume = False
# Select the model you want to fine tune
what_do_you_want = 'densenet_maxpooling_relu'

# _search_space = {'input_dim': 3 * image_shape[0] * image_shape[1],
#                  'num_classes': 1,
#                  'hidden1': hp.uniformint('hidden1', 200, 1000),
#                  # 'hidden1': 500,
#                  'hidden2': 200,
#                  'hidden3': 100,
#                  'dropout': 0.3,
#                  # 'num_filters1': hp.uniformint('num_filters1', 10, 50),
#                  'num_filters1': None,
#                  # 'num_filters2': hp.uniformint('num_filters2', 10, 50),
#                  'num_filters2': None,
#                  'input_channels': 3,
...
_search_space = {'input_dim': 3 * image_shape[0] * image_shape[1],
                 'num_classes': 1,
                 'hidden1': hp.choice('hidden1', [250, 500, 750]),
                 'hidden2': 200,
                 'hidden3': 100,
                 'dropout': 0.3,
                 'num_filters1': None,
...
trials = Trials()
# Set the number of trials hyperopt will do
max_evals = 9

# Import partial if you want to pass arguments to a function in fmin
from functools import partial

# Call fmin from hyperopt to fine tune the model in hte search space given
...
with open('/mnt/data/sotiris/checkpoints/{}/{}_hyperopt_trials_best.p'.format(what_do_you_want, what_do_you_want),
          'wb') as out:
